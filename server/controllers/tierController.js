...
const createTier = async (req, res) => {
  const { tier_name, tier_description, permissions } = req.body;
...
const getTiers = async (req, res) => {
  try {
    const [tiers] = await db.execute(`
      SELECT t.id, t.tier_name, t.description as tier_description, GROUP_CONCAT(p.permission_name) AS permissions
      FROM tiers t
...
const assignTierToUser = async (req, res) => {
  ...
  try {
    ...
    const adminRoles = [1]; // Replace with actual admin role ids
